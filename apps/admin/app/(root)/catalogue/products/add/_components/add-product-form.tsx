...
import { addProductFormSchema } from "@/lib/zod"
...
import { BrandForSelect, CategoryForSelect } from "@/types"
...
const AddProductForm = ({ categories, brands }: AddProductFormProps) => {
  ...
  const onSubmit: SubmitHandler<z.infer<typeof addProductFormSchema>> = async (productData) => {
  ...
  const handleDiscard = async () => {
    startTransition(async () => {
      ...
      const filesToBeDeleted = [...formValues.images]

      if (formValues.variants?.length) {
        formValues.variants.map((v) => {
          if (v.images.length) {
            filesToBeDeleted.push(...v.images)
          }
        })
      }

      if (formValues.documents.length) {
        formValues.documents.map((d) => {
          filesToBeDeleted.push(d.url)
        })
      }
            ...
            <div className="grid auto-rows-max gap-y-4">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Attach product related documents.</CardDescription>
                </CardHeader>
                <CardContent>
                  <DocumentField
                    control={form.control}
