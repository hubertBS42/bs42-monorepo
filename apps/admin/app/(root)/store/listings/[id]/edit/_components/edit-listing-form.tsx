...
import { updateListingFormSchema } from "@/lib/zod"
...
const DeleteListing = dynamic(() => import("./delete-listing"), {
  ssr: false,
...
const EditListingForm = ({ listing }: EditListingFormProps) => {
  ...
  const form = useForm<z.infer<typeof updateListingFormSchema>>({
    ...
    defaultValues: {
      ...
      compareAtPrice: Number(listing.compareAtPrice),
      stock: listing.stock,
      lowStockThreshold: listing.lowStockThreshold,
      trackInventory: listing.trackInventory,
      status: listing.status as Status,
      isFeatured: listing.isFeatured,
      variants: listing.storeListingVariants.map((slv) => ({
        variantId: slv.variantId,
        sellPrice: Number(slv.sellPrice),
        buyPrice: Number(slv.buyPrice),
        compareAtPrice: Number(slv.compareAtPrice),
        stock: slv.stock,
        lowStockThreshold: slv.lowStockThreshold,
        trackInventory: slv.trackInventory,
        status: slv.status as Status,
      })),
    },
  })

  const onSubmit: SubmitHandler<z.infer<typeof updateListingFormSchema>> = async (data) => {
    startTransition(async () => {
      const response = await updateListingAction(data)
      if (!response.success) {
    ...
    <form onSubmit={form.handleSubmit(onSubmit)}>
      <div className="grid gap-y-6">
        <ResourceFormHeader
          ...
          description="Update your store's listing for this product."
            ...
            <div className="grid auto-rows-max gap-y-4 lg:col-span-2">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Your store&apos;s prices for this product.</CardDescription>
                </CardHeader>
                <CardContent>
                  <FieldGroup>
              ...
              {!hasVariants && (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Stock levels for this listing.</CardDescription>
                  </CardHeader>
                  <CardContent>
              ...
              {hasVariants && (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Per-variant pricing and stock. Leave prices empty to use the listing default.</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <Table>
                      <TableHeader>
            ...
            <div className="grid auto-rows-max gap-y-4">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Remove this product from your store.</CardDescription>
                </CardHeader>
                <CardContent>
