...
const getNavItems = (role: SystemRole, isGlobalWorkspace: boolean): Record<string, NavItem[]> => {
  ...
  return (
    Object.entries(routePermissions)
      ...
      .map(([url, permission]) => {
        const filteredSubItems = permission.nav?.items?.filter((item) => !item.context || item.context.includes(currentContext))
...
export const useNavItems = (): UseNavItemsReturn => {
  ...
  const navItems = getNavItems(session.user.role as SystemRole, isGlobalWorkspace)
